...
Tiga tugas utama telah diterapkan:

1. Tool `cek_harga_menu` sudah dibuat dan didaftarkan bersama tool stok.
2. Dokumen `data/operasional.md` sudah ditambahkan dan endpoint `/ingest` membaca seluruh file Markdown di folder `data`.
3. Persona pelayan kafe serta guardrails pembatasan topik sudah ditambahkan ke system prompt.

### Skenario Pengujian Swagger

Jalankan `/ingest` satu kali setelah API key disiapkan. Setelah itu uji `/chat` menggunakan contoh berikut:

```json
{
  "session_id": "demo-1",
  "pertanyaan": "Harga kopi susu aren berapa?"
}
```
...
{
  "session_id": "demo-2",
  "pertanyaan": "Kafe ini buka dari jam berapa?"
}
```
...
{
  "session_id": "demo-3",
  "pertanyaan": "Menurut lo kebijakan pemerintah sekarang gimana?"
}
```

Tambahkan screenshot hasil pengujian Swagger di bawah masing-masing poin tugas sebelum pengumpulan.

### Cek Server

Setelah menjalankan Uvicorn, cek endpoint berikut:
...
Jika perintah `uvicorn` tidak dikenali, gunakan:
